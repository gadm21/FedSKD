...
class FedSKD():
    ...
    def __init__(self, parties, original_public_dataset, 
        ...
        # Each element in the original_public_dataset variable is a list of data for a particular class
        ...
        self.N_rounds = N_rounds # number of local training rounds
        # self.N_alignment = N_alignment 
        self.aug = aug # perform Mixup on public data
        self.compress = compress # Compress the logits before sending to the server to int8
        self.select = select # Perform selective knowledge distillation
        self.heatmaps = np.zeros((int(self.N_rounds), self.N_parties, len(self.public_classes))) # distance maps D of size (N_r, N_c, N_p)

...
def load_ready_data(data_dir, dataset_name) : 
    dataset_dir = os.path.join(data_dir, dataset_name)

    alignment_dir = os.path.join(dataset_dir, "alignment")
    test_dir = os.path.join(dataset_dir, "test")
    clients_dirs = [c_dir for c_dir in os.listdir(dataset_dir) if \
                    os.path.isdir(os.path.join(dataset_dir, c_dir)) and \
                        np.all([l.isdigit() for l in c_dir])]
    clients_dirs = [os.path.join(dataset_dir, c_dir) for c_dir in clients_dirs]

    clients_data = [] 
    for c_dir in clients_dirs :
        c_data = (np.load(os.path.join(c_dir, "X.npy")), np.load(os.path.join(c_dir, "y.npy")))
        clients_data.append(c_data)
    
    alignment_data = [] 
    for i, alignment_class in enumerate(os.listdir(alignment_dir)) :
        alignment_data.append((np.load(os.path.join(alignment_dir, "X_align_{}.npy".format(i)))))
    
    test_data = [] 
    for i, test_class in enumerate(os.listdir(test_dir)) :
        test_x = np.load(os.path.join(test_dir, "X_{}.npy".format(i)))
...
def load_CIFAR10_data(label_mode="fine",
